...
global front_frame_1_x, front_frame_1_y, front_frame_2_x, front_frame_2_y
front_frame_1_x=[]
front_frame_2_x=[]
front_frame_1_y=[]
front_frame_2_y=[]
    ...
    def estimate_paf(peaks, heat_mat, paf_mat):
        ...
        for human_id in range(pafprocess.get_num_humans()):
            ...
            for part_idx in range(18):
                ...
                #print(heat_mat.shape[1],heat_mat.shape[0])

                is_added = True
                human.body_parts[part_idx] = BodyPart(
                    '%d-%d' % (human_id, part_idx), part_idx,
                    float(pafprocess.get_part_x(c_idx)) / heat_mat.shape[1],    # after normalize
                    float(pafprocess.get_part_y(c_idx)) / heat_mat.shape[0],    # after normalize
...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_humans(npimg, npimg2, humans, frame_num, imgcopy=False, skeletonornot=True):

        if skeletonornot is False:
            return npimg2
        ...
        with open('./tf_pose/output.csv', 'a', newline='') as csvfile:
            ...
            # load model 
            classifier2 = load('./tf_pose/model2.pkl') 
            classifier1 = load('./tf_pose/model1.pkl')
            yolo = 0
            for human in humans:
                ...
                for pair_order, pair in enumerate(common.CocoPairsRender):
                    ...
                    npimg2 = cv2.line(npimg2, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                ...
                now_frame_x=[]
                now_frame_y=[]
                # construct now frame list
                for item in human.body_parts.items():
                    now_frame_x.append(item[1].x)
                    now_frame_y.append(item[1].y)
                
                
                if len(humans) >= 2:   # two points detected judge human id
                    h1 = humans[0]
                    h2 = humans[1]
                    distance1 = 0
                    distance2 = 0
                    human_id = ""
                    body_parts_num = min (len(h1.body_parts) ,len(h2.body_parts))
                    
                    for i in range(common.CocoPart.Background.value):
                        
                        if i not in h1.body_parts.keys():
                            continue

                        if i > body_parts_num:
                            break

                        body_part = h1.body_parts[i]
                        center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
                        centers[i] = center
                        distance1 += center[1]
                    ...
                    for i in range(common.CocoPart.Background.value):
                        ...
                        if i not in h2.body_parts.keys():
                            continue
                            
                        if i > body_parts_num:
                            break

                        body_part = h2.body_parts[i]
                        center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
                        centers[i] = center
                        distance2 += center[1]

                    # y value bigger is assing to player id 1
                    if yolo==0 :
                        ...
                        if distance1 > distance2:
                            ...
                            human_id = "1"   
                else :  # frame lose judge human id
                    ...
                    # initialize front frame if size = 0  
                    if len(front_frame_1_x)==0 and len(now_frame_x) > 2:
                        front_frame_1_x = now_frame_x
                        front_frame_1_y = now_frame_y
                    elif len(front_frame_2_x)==0 and len(now_frame_x) > 2:
                        front_frame_2_x = now_frame_x
                        front_frame_2_y = now_frame_y 
                            
                    # count the distance between now frame and front frame
                    if len(front_frame_1_x) > 0 and len(front_frame_2_x) > 0:

                        for item in human.body_parts.items():
                            if len(front_frame_1_x)-1 < id or len(front_frame_2_x)-1 < id or len(front_frame_1_y)-1 < id or len(front_frame_2_y)-1 < id:
                                break
                            distance1 += math.pow(float(item[1].x) - float(front_frame_1_x[id]),2) + math.pow(float(item[1].y)-float(front_frame_1_y[id]),2)
                            distance2 += math.pow(float(item[1].x) - float(front_frame_2_x[id]),2) + math.pow(float(item[1].y)-float(front_frame_2_y[id]),2)
                            id+=1
                            if(id>=18):
                                break
                                
                    # judge the human id by distance 
                    if(distance1 < distance2):
                        human_id = "1"
                        ...
                        human_id = human_count       


                # writerow to csv file
                test_input=[]
                point_position = str(frame_num) + "," + str(human_id)+","
                pred = None
                
                for item in human.body_parts.items():
                    #item is a tuple with item[0] being the ID and item[1] being class instance BodyPart                    
                    point_position = point_position + str(item[1].x) + ","+ str(item[1].y) + ","

                    if len(test_input)<28:
                        test_input.append(item[1].x)
                        test_input.append(item[1].y)
                ...
                if len(test_input) == 28:
                    ...
                    test_np = test_np.reshape(-1, 28)

                elif len(test_input) < 28:
                    an_array = np.array(test_input)
                    test_np = np.zeros((1, 28))
                    test_np[:,:len(test_input)] = an_array

                if human_id == "2":
                    pred = classifier2.predict(test_np)
                elif human_id == "1":
                    pred = classifier1.predict(test_np)

                writer.writerow(point_position.split(','))
                point_position=""
                ...
                rec_diag = 50
                if human_id == "1":
                    cv2.rectangle(npimg, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), (int(now_frame_x[0]*x_width)-rec_diag, int(now_frame_y[0]*y_width)-rec_diag*2), (255, 0, 0), 2) 
                    text="player_id =  " + str(human_id) 
                    if pred is not None:
                        text = text + " " + str(pred)
                    cv2.putText(npimg, text, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), cv2.FONT_HERSHEY_SIMPLEX,0.5, (255, 0, 0), 1, cv2.LINE_AA)
                if human_id == "2":
                    cv2.rectangle(npimg, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), (int(now_frame_x[0]*x_width)-rec_diag, int(now_frame_y[0]*y_width)-rec_diag*2), (0, 255, 0), 2) 
                    text="player_id =  " + str(human_id) 
                    if pred is not None:
                        text = text + " " + str(pred)
                    cv2.putText(npimg, text, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), cv2.FONT_HERSHEY_SIMPLEX,0.5, (0, 255, 0), 1, cv2.LINE_AA)
                '''
                x_width=640
                y_width=500
                rec_diag = 50
                if human_id == "1":
                    #cv2.rectangle(npimg2, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), (int(now_frame_x[0]*x_width)-rec_diag, int(now_frame_y[0]*y_width)-rec_diag*2), (255, 0, 0), 2) 
                    text="player_id =  " + str(human_id) 
                    if pred is not None:
                        text = text + " " + str(pred)
                    cv2.putText(npimg2, text, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), cv2.FONT_HERSHEY_SIMPLEX,0.5, (255, 0, 0), 1, cv2.LINE_AA)
                if human_id == "2":
                    #cv2.rectangle(npimg2, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), (int(now_frame_x[0]*x_width)-rec_diag, int(now_frame_y[0]*y_width)-rec_diag*2), (0, 255, 0), 2) 
                    text="player_id =  " + str(human_id) 
                    if pred is not None:
                        text = text + " " + str(pred)
                    cv2.putText(npimg2, text, (int(now_frame_x[0]*x_width)+rec_diag, int(now_frame_y[0]*y_width)+rec_diag*2), cv2.FONT_HERSHEY_SIMPLEX,0.5, (0, 255, 0), 1, cv2.LINE_AA)
                ...
                # if frame is not detect add another frame
                loseframe=[]
                if len(humans) < 2 and human_id == "1":
                    loseframe.append(str(frame_num))
                    loseframe.append("2")
                    writer.writerow(loseframe)
                elif len(humans) < 2 and human_id == "2":
                    loseframe.append(str(frame_num))
                    loseframe.append("1")
                    writer.writerow(loseframe)
