...
while set_num <= 3:
	...
	while score_A < 22:
		...
		infile_video = dir_path + rally_score + ".mp4"
		if os.path.isfile(infile_predict) == True and os.path.isfile(infile_video) == True:
			...
			c = 1
			path = dir_path + rally_score + '_predict_snapshot'
			if vc.isOpened():
				rval, frame = vc.read()
			...
			while rval:
				if predict_result and c == predict_result[0]:
					cv2.imwrite(dir_path + rally_score + '_predict_snapshot/snapshot'+ str(c) + '.jpg',frame)
		...
		if os.path.isfile(infile_out) == True and os.path.isfile(infile_video) == True:
			...
			vc = cv2.VideoCapture(infile_video)
			c = 1
			path = dir_path + rally_score + '_out_snapshot'
			if vc.isOpened():
				rval, frame = vc.read()
				if not os.path.exists(path):
					os.makedirs(path)
			...
			while rval:
				if ground_truth and c == ground_truth[0]:
					cv2.imwrite(dir_path + rally_score + '_out_snapshot/snapshot'+ str(c) + '.jpg',frame)
					ground_truth.pop(0)
				c = c + 1
				rval, frame = vc.read()
