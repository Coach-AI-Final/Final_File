...
peaks, properties = find_peaks(y, prominence=1)#distance=10) #y是吃非0,所以index有跳
...
for i in range(len(y)-2,2,-1):
    if((y[i+1] - y[i]) / (z[i+1] - z[i]) >= 4 and (y[i] - y[i-1]) / (z[i] - z[i-1]) >= 4 and (y[i-1] - y[i-2]) / (z[i-1] - z[i-2]) >= 4):
        end_point = i+front_zeros[i]
        print(int(end_point))
        break
...
for i in range(len(peaks)-1):
    ...
    upper=[]
    plt.plot(z[start:end],y[start:end],'-')#plt.plot(frames[start:end],realy[start:end],'-')
    lower = np.argmin(y[start:end]) #找到最低谷(也就是從最高點開始下墜到下一個擊球點),以此判斷扣殺或平球軌跡
    for j in range(start+lower+5, end+1-5):
        if(j-(start+lower) > 5) and (end - j > 5):
            if (y[j] - y[j-1])*4 < (y[j+1] - y[j]):
                print(j, end=',')
...
for i in range(len(frames)):
    if i in final_predict:
        plt.plot(frames[i], realy[i],'x', markersize=12)

with open(filename[:-4] + '_shot.csv','w', newline='') as csvfile1:
    h = csv.writer(csvfile1)
    h.writerow(['frame','hit'])
    for i in range(len(frames)):
        if i in final_predict:
